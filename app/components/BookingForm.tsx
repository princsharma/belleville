...
export default function BookingForm() {
  ...
  return (
    <section id="book" className="relative overflow-hidden bg-gradient-to-b from-white via-brand-green-soft/30 to-brand-green-soft/60">
            ...
            <div className="relative">
              ...
              <div className="relative bg-white rounded-3xl border border-brand-gray-line shadow-card-lg overflow-hidden">
                ...
                <div className="bg-brand-gradient p-6 lg:p-8 text-white">
                  ...
                  <p className="mt-2 text-sm text-white/90">A patient coordinator will reach out within 15 minutes.</p>
                </div>
                  ...
                  {submitted ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      ...
                      <label className="flex items-start gap-3 text-sm text-brand-dark pt-2 cursor-pointer">
                        <input type="checkbox" required className="mt-1 accent-[#059669] w-4 h-4" />
                        <span>
                          I accept the{' '}
                          <a href="#" className="text-brand-green underline underline-offset-2 font-medium">
                            Terms and Conditions
                          </a>
                        </span>
                      </label>
                      <label className="flex items-start gap-3 text-sm text-brand-slate cursor-pointer">
                        <input type="checkbox" className="mt-1 accent-[#059669] w-4 h-4" />
                      ...
                      <button
                        type="submit"
                        disabled={submitting}
                        className="btn-primary w-full justify-center mt-2 text-base py-3.5 disabled:opacity-60"
                      >
                      ...
                      <p className="text-xs text-brand-gray text-center mt-3 leading-relaxed flex items-center justify-center gap-1.5">
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                          <path d="M3 5V3.5a3 3 0 016 0V5M2.5 5h7v6h-7z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
...
function Field({
  ...
  return (
    <div>
      ...
      <input
        {...props}
        className="w-full bg-white border border-brand-gray-line rounded-xl px-4 py-3 text-[15px] text-brand-dark placeholder:text-brand-gray/70 focus:outline-none focus:ring-2 focus:ring-brand-green/30 focus:border-brand-green transition-all"
      />
      {helper && <p className="mt-1.5 text-xs text-brand-gray">{helper}</p>}
    </div>
  )
}
